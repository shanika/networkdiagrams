  ...
  * ====================== */
  ...
  MultiSelect.prototype = {
    ...
    init: function(){
      ...
      if (ms.next('.ms-container').length == 0){
        ...
        var optgroupLabel = null,
            ...
            optgroupLiTemplate = '<li class="ms-optgroup-label"></li>';
        
        var selectHandler = that.options.selectHandler? that.options.selectHandler:""; 
        ms.find('optgroup, option').each(function(){
          if ($(this).is('optgroup')){
            ...
            if (that.options.selectableOptgroup){
            	if(selectHandler){
                    optgroupSelectableLi.on('click', selectHandler, function(){
                        var values = optgroup.children(':not(:selected)').map(function(){ return $(this).val() }).get();
                        that.select(values);
                      });

                      optgroupSelectionLi.on('click',selectHandler, function(){
        ...
        if(that.options.selectableTitle){
        	that.$selectableContainer.append(selectableHeaderTemplate);
        }
        ...
        if(that.options.selectAllButtonId){
        	selectAllButton = selectAllButton.replace('select-all',that.options.selectAllButtonId )
        	that.$selectableContainer.append(selectAllButton);
        }
        
        if(that.options.selectionTitle){
        	that.$selectionContainer.append(selectionHeaderTemplate);
        }
        ...
        if(that.options.deselectAllButtonId){
        	deselectAllButton = deselectAllButton.replace('deselect-all',that.options.deselectAllButtonId )
        	that.$selectionContainer.append(deselectAllButton);
        }
        that.$container.append(that.$selectableContainer);
        that.$container.append('<div class="pac-middle"></div>');
        ...
        var eventHandle = 'click';//by default
       
        if(selectHandler){
        	that.$selectableUl.on('mouseover vmouseover', '.ms-elem-selectable', function(){
	            $(this).find(selectHandler).show();
	        });
        	that.$selectableUl.on('mouseout vmouseout', '.ms-elem-selectable', function(){
	            $(this).find(selectHandler).hide();
	        });
	         that.$selectionUl.on('mouseover vmouseover', '.ms-elem-selection', function(){
	        	$(this).find(selectHandler).show();
	        });
	        that.$selectionUl.on('mouseout vmouseout', '.ms-elem-selection', function(){
		       	$(this).find(selectHandler).hide();
		    });
        	that.$selectableUl.on(eventHandle,selectHandler, function(){
	            that.select($(this).parents('.ms-elem-selectable').data('ms-value'));
	          });
	          that.$selectionUl.on(eventHandle, selectHandler, function(){
	            that.deselect($(this).parents('.ms-elem-selection').data('ms-value'));
	          });
        }else{
        	 if(that.options.dblClick) {
             	eventHandle = 'dblclick';
             }  
        	that.$selectableUl.on(eventHandle, '.ms-elem-selectable', function(){
	            that.select($(this).data('ms-value'));
	          });
	          that.$selectionUl.on(eventHandle, '.ms-elem-selection', function(){
    ...
    'select' : function(value, method){
      ...
      if(that.options.selectHandler){
    	  selectables.find(that.options.selectHandler).hide();
    	  selections.find(that.options.selectHandler).hide();
      }
    },
